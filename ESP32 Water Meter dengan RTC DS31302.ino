...
#define RTC_CLK_PIN 5   // CLK pin
#define RTC_DAT_PIN 4   // DAT pin  
#define RTC_RST_PIN 2   // RST pin
...
const float ML_PER_PULSE = 1.0f / 450.0f;
...
// Convert RtcDateTime to time_t
time_t rtcDateTimeToTimeT(const RtcDateTime& dt) {
  struct tm timeinfo;
  ...
  return localTime - gmtOffset_sec;
}

// Convert time_t to RtcDateTime
RtcDateTime timeTToRtcDateTime(time_t t) {
  time_t localTime = t + gmtOffset_sec;
...
bool initRTC() {
  ...
  if (!rtc.IsDateTimeValid()) {
    Serial.println("[RTC] DS1302 RTC lost confidence in the DateTime!");
    // Set RTC to compile time if not valid
    RtcDateTime compiled = RtcDateTime(__DATE__, __TIME__);
    rtc.SetDateTime(compiled);
    Serial.println("[RTC] Setting compile time to DS1302");
  }
  
  // Enable RTC if it was disabled
  if (!rtc.GetIsRunning()) {
    Serial.println("[RTC] DS1302 was not actively running, starting now");
  ...
  Serial.println("[RTC] DS1302 initialized successfully!");
...
void syncRTCWithNTP() {
  ...
  if (ntpTime > 1577836800) {
    RtcDateTime ntpDateTime = timeTToRtcDateTime(ntpTime);
...
bool setRTCTime(const String& timeStr) {
  ...
  if (valueIndex == 6) {
    RtcDateTime newTime(values[0], values[1], values[2], values[3], values[4], values[5]);
...
bool isValidRTCTime(const RtcDateTime& dt) {
...
void processBTCommand(const String& cmd) {
  ...
  if (cmd == "RESET_LOG") {
    ...
    if (rtcInitialized) {
      RtcDateTime now = rtc.GetDateTime();
      if (isValidRTCTime(now) && rtc.IsDateTimeValid()) {
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("\n[SETUP] Starting Water Meter with DS1302 RTC...");
  ...
  if (!SerialBT.begin("WaterMeter_DS1302")) {
