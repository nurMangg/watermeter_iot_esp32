...
void flushBuffer() {
  ...
  if (f) {
    f.print(String(totalVolume, 3)); // presisi 3 angka di belakang koma
    f.close();
    Serial.println("[FS] Total volume disimpan: " + String(totalVolume, 3) + " L");
  } else {
    Serial.println("[FS] Gagal menyimpan total volume.");
  }
}
...
void loadTotalVolume() {
  if (!LittleFS.exists("/total.txt")) {
    Serial.println("[FS] File total.txt tidak ditemukan. Menginisialisasi ke 0.");
    totalVolume = 0.0f;
    return;
  }
  ...
  Serial.println("[FS] Total volume dimuat dari file: " + String(totalVolume, 3) + " L");
}
